...
describe Pawn do
  ...
  describe 'local_move_valid?' do
    ...
    let(:valid?) { pawn.local_move_valid?(start: start, goal: goal) }

    context 'when pawn not moved and goal is 1 square from start and it is empty' do
      let(:color) { 'white' }
      subject(:pawn) { described_class.new(board: board, color: color) }

      it 'is valid' do
        start = [0, 0]
        goal = [1, 0]
        allow(board).to receive(:empty_at?).with(goal).and_return(true)
        expect(pawn.local_move_valid?(start: start, goal: goal)).to be true
      end
    end
    ...
    context 'when pawn not moved and goal is 1 square from start and it is not empty' do
      ...
      it 'is not valid' do
        ...
        expect(pawn.local_move_valid?(start: start, goal: goal)).to be false
      end
    end
    ...
    context 'when pawn moved and goal is 1 square from start and it is empty' do
      let(:color) { 'white' }
      subject(:pawn) { described_class.new(board: board, color: color, moved: true) }
