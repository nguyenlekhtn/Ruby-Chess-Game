...
describe King do
  ...
  describe 'local_move_valid?' do
    ...
    let(:valid_start_goal?) { king.local_move_valid?(start: start, goal: goal) }

    context 'if goal is right after start in ranks' do
      it 'returns true' do
        start = [1, 1]
        goal = [1, 2]
        expect(king.local_move_valid?(start: start, goal: goal)).to be true
      end
    end
    ...
    context 'if goal is in same ranks but distance = 2' do
      it 'returns false' do
        start = [3, 3]
        goal = [3, 5]
        expect(king.local_move_valid?(start: start, goal: goal)).to be false
      end
    end
